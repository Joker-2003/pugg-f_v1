...
const FinishScreen = ({ newHigh, onRestart, onReset, result }) => (
  <div className="info-screen">
    ...
    <button className="mole-btn" onClick={() =>{ window.location.href = "http://localhost:3000/menu"}}>Main Menu</button>
  </div>
)
...
const StartScreen = ({ onStart }) => {
  ...
  return (
    <div className="info-screen">
      ...
      <button className="mole-btn" onClick={onStart}>Start Game</button>
      <button className="mole-btn" onClick={() => {
      ...
      <svg
        ...
        <g clipPath="url(#mole-clip)">
          <foreignObject x={0} y={0} width={200} height={200}>
            <button ref={buttonRef} onClick={whack} className="mole-btn mole__whack">
              Whack!
            </button>
          </foreignObject>
        </g>
...
const Game = (props) => {
  ...
  const [highScore, setHighScore] = useState(props.user.all_1)
  ...
  const endGame = async () => {
    ...
    if (score > parseInt(highScore, 10)) {
      ...
      let HIGH_REQ = await updateScore(1, score)
    }
    setPlaying(false)
    setFinished(true)
  }
  ...
  return (
    <Fragment>
      ...
      {!starting && !playing && !finished && <HiScore value={props.user.all_1} />}
      {/* Fresh */}
...
export default function WackAMole(props) {



  return (
    <div className="wack-wack"><Game user = {props.user} /></div>
  )
}